...
public class ChangeService {
  ...
  public List<Method> fetchChangeSets(final Class<?> type) throws CouchVersionChangeSetVersionException {
  ...
  private List<Method> filterChangeSetAnnotation(List<Method> allMethods) throws CouchVersionChangeSetVersionException {
    ...
    for (final Method method : allMethods) {
      if (method.isAnnotationPresent(ChangeSet.class)) {
        ...
        if (changeSetIds.contains(id)) {
          throw new CouchVersionChangeSetVersionException(String.format("Duplicated changeset id found: '%s'", id));
        }
        changeSetIds.add(id);
